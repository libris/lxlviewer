...
<script lang="ts">
	...
	let shouldInitObserver = $derived(true);
	...
	$effect(() => {
		if (shouldInitObserver && searchContainerElement) {
			initObserver();
		}
	});

	onNavigate((navigation) => {
		if ((navigation.to?.scroll?.y || 0) > (searchContainerElement?.offsetTop || 0)) {
