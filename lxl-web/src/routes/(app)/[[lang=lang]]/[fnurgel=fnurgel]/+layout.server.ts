...
export const load = async ({ url, fetch, params }) => {
	...
	const citationId = url.searchParams.get('cite');
	const locale = getSupportedLocale(params?.lang);

	if (citationId) {
		return { citations: getCitationData(citationId, locale) };
	}
	return {};

	async function getCitationData(citationId: string, locale: LocaleCode) {
		let data;
		let error;

		// fetching the record again via cite api is needed as long as csl mapping depends on computedLabel
		const res = await fetch(`/api/${locale}/cite?id=${citationId}&format=csl`);
		if (res.ok) {
