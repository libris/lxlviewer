<script lang="ts">
	...
	function addQualifierKey(key: string) {
		if (searchContext.superSearch) {
			const insert = ` ${key}:()`;
			const anchor =
				(searchContext.superSearch.getActiveEditorView()?.state.doc.length || 0) +
				insert.length -
				1;
			const head = anchor;
			searchContext.superSearch.dispatchChange({
				change: {
					insert
				},
				selection: {
...
<article class="mx-auto mt-4 mb-12 w-7xl p-4 sm:px-6 lg:mt-8">
	{#if data.locale === 'en'}
		<EnContent />
	{:else}
		<SvContent />
