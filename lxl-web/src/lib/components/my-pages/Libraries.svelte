...
<div class="page-padding mx-auto mt-8 mb-12 w-screen max-w-full md:max-w-5xl">
	...
	>
		<div class="md:w-3/5">
			...
			<div class="relative">
				<BiSearch class="absolute top-6 left-2.5 text-sm" />
				<input
				...
				<div>
					{#if search.data}
						...
						{#if searchResult?.items && searchResult?.items.length !== 0}
							<div class="my-3 rounded-md py-2">
								{#each searchResult.items as resultItem (resultItem['@id'])}
									<div class="flex min-h-12 w-full items-center justify-between">
		...
		<div class="md:ml-10 md:w-80">
			<div>{page.data.t('myPages.favouriteLibraries')}</div>
			<div class="py-2">
