...
| `shallowRouting` | `boolean` | Controls if the component should support the expanded state using the [History API](https://developer.mozilla.org/en-US/docs/Web/API/History_API) (e.g. closing the dialog when pressing back). Links in the expanded search are intercepted so they first go back (collapsing the dialog) before navigating to the `href` attribute of the link. The same applies to form submissions. TODO: Programmatic usage of `goto` should also intercepted similarly. | `false` |
...
- `resultItem`- An individual item of the resulting data from `queryFn` and `transformFn`. The data inside can be of arbitary shape so they can be rendered in any shape you want.
