...
| `shallowRouting` | `boolean` | Controls if the component should support the expanded state using the [History API](https://developer.mozilla.org/en-US/docs/Web/API/History_API) (e.g. closing the dialog when pressing back). Navigation from the expanded dialog should use `replaceState` if `shallowRouting` is enabled (otherwise back navigations will point to the expanded state) | `false` |
