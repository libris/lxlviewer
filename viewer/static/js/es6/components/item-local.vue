...
<template>
  <div class="item-local-container">
    <div v-if="!isExpandedType" class="item-local" :class="{'expanded': expanded}">
      ...
      <data-node v-show="expanded && k !== '_uid' && v" v-for="(k,v) in filteredItem" :parent-path="getPath" :entity-type="item['@type']" :is-inner="true" :is-locked="isLocked" :allow-local="true" :is-removable="false" :embedded="true" :parent-key="key" :parent-index="index" :key="k" :value="v" :focus="focus" :show-action-buttons="showActionButtons"></data-node>
    </div>
