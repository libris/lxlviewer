...
export default {
  ...
  computed: {
    ...
    options() {
      let sortOptions = this.settings.sortOptions[this.commonBaseType];
      if (sortOptions) {
        return sortOptions;
      }
      if (this.commonSortFallback) {
        sortOptions = this.settings.sortOptions.Common;
      }
      if (sortOptions) {
        return sortOptions;
      }
    ...
    commonBaseType() {
      ...
      if (Array.isArray(this.recordTypes)) {
        const baseTypes = this.recordTypes.reduce((accumulator, currType) => {
          const baseType = VocabUtil.getRecordType(currType, this.vocab, this.context);
